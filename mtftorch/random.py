...
from mtftorch._C import default_generator
...
def manual_seed(seed) -> torch._C.Generator:
    ...
    """
    seed = int(seed)
    import mtftorch.cuda

    if not mtftorch.cuda._is_in_bad_fork():
        mtftorch.cuda.manual_seed_all(seed)
...
def fork_rng(devices=None, enabled=True, _caller="fork_rng", _devices_kw="devices"):
    ...
    if devices is None:
        num_devices = mtftorch.cuda.device_count()
        if num_devices > 1 and not _fork_rng_warned_already:
            warnings.warn(
                ("CUDA reports that you have {num_devices} available devices, and you "
                 ...
                 "to `range(mtftorch.cuda.device_count())`."
    ...
    for device in devices:
        gpu_rng_states.append(mtftorch.cuda.get_rng_state(device))
    ...
    try:
        ...
        for device, gpu_rng_state in zip(devices, gpu_rng_states):
            mtftorch.cuda.set_rng_state(gpu_rng_state, device)