...
from mtftorch import Tensor
from mtftorch.nn.parameter import Parameter #, UninitializedParameter
...
class Identity(Module):
    ...
    Examples::
        ...
        >>> input = mtftorch.randn(128, 20)
        >>> output = m(input)
        >>> print(output.size())
        mtftorch.Size([128, 20])

    """
    def __init__(self, scope='identity', **kwargs):
        super(Identity, self).__init__(scope=scope, **kwargs)
        with self.scope():
            pass

    def forward(self, input: Tensor) -> Tensor:
        with self.scope():
            input = mtf.import_tf_tensor(mtftorch.get_mesh(), input)
            return mtf.identity(input)
